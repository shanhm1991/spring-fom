...
public class LoggerAppender extends FileAppender{
	...
	protected int maxBackupIndex = 50;

	private long logExistDay = 7;

	private long tarExistDay = 180;
	...
	public void rename(String date) {
		...
		for (; index <= maxBackupIndex; ++index) {
			if (!(bak = new File(fileName + "." + date + "." + index)).exists()) {
				break;
			}
		}
		if(index == 1){
	...
	private static class InnerThread extends Thread {
		...
		private long logExistDay = 3;

		private long tarExistDay = 30;
			...
			for(File file : array){
				...
				if(name.endsWith(".tar.gz")){
					...
					try {
						...
						long existDay = (now - time) / 86400000;
